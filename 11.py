...
def helper(row, column, input, visited):
    ...
    if row >= len(input):
        return 0
    elif row + 3 < len(input):
        column_product = input[row][column] * input[row + 1][column] * input[row + 2][column] * input[row + 3][column]
        max_vertical = max(column_product, helper(row + 1, column, input, visited))
    else:
        max_vertical = 0

    if column >= len(input[row]):
        return 0
    elif column + 3 < len(input[row]):
        row_product = input[row][column] * input[row][column + 1] * input[row][column + 2] * input[row][column + 3]
        max_horizontal = max(row_product, helper(row, column + 1, input, visited))
    else:
        max_horizontal = 0

    if row + 3 < len(input) and column + 3 < len(input[row]):
        diagonal_product = input[row][column] * input[row + 1][column + 1] * input[row + 2][column + 2] * input[row + 3][column + 3]
        max_diagonal = max(diagonal_product, helper(row + 1, column + 1, input, visited))
        #max_diagonal = diagonal_product
    else:
        max_diagonal = 0


    return max(max_vertical, max_horizontal, max_diagonal)
